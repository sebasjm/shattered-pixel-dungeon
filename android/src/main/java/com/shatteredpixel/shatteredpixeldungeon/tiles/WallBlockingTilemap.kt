...
class WallBlockingTilemap : Tilemap("wall_blocking.png", TextureFilm("wall_blocking.png", SIZE, SIZE)) {

    private var curr: Int = 0

    init {
        map(IntArray(Dungeon.level!!.length()), Dungeon.level!!.width())
    }

    @Synchronized
    override fun updateMap() {
        super.updateMap()
        data = IntArray(size) //clears all values, including cleared tiles
    ...
    private fun fogHidden(cell: Int): Boolean {
    ...
    private fun door(cell: Int): Boolean {
        return DungeonTileSheet.doorTile(Dungeon.level!!.map!![cell])
    }

    @Synchronized
    fun updateArea(cell: Int, radius: Int) {
        val l = cell % mapWidth - radius
        val t = cell / mapWidth - radius
        val r = cell % mapWidth + radius
        val b = cell / mapWidth + radius
        updateArea(
                Math.max(0, l),
                Math.max(0, t),
    ...
    @Synchronized
    fun updateArea(x: Int, y: Int, w: Int, h: Int) {
        var cell: Int
        for (i in x..x + w) {
            for (j in y..y + h) {
                cell = i + j * mapWidth
                if (cell < data.size && data[cell] != CLEARED)
                    updateMapCell(cell)
            }
        }
    }
    ...
    companion object {
        ...
        private val BLOCK_NONE = -1
        private val BLOCK_RIGHT = 0
        private val BLOCK_LEFT = 1
        private val BLOCK_ALL = 2
        private val BLOCK_BELOW = 3
    }
}
