...
open class TunnelRoom : ConnectionRoom() {
    ...
    protected open val connectionSpace: Rect
        get() {
            val c = doorCenter
    ...
    protected val doorCenter: Point
        get() {
            ...
            c.x = GameMath.gate((left + 1).toFloat(), c.x.toFloat(), (right - 1).toFloat()).toInt()
            c.y = GameMath.gate((top + 1).toFloat(), c.y.toFloat(), (bottom - 1).toFloat()).toInt()
