...
abstract class RegularLevel : Level() {
    ...
    protected open fun initRooms(): ArrayList<Room> {
        ...
        val specials = specialRooms() + 2
        SpecialRoom.initForFloor()
        for (i in 0 until specials)
            initRooms.add(SpecialRoom.createRoom())

        val secrets = SecretRoom.secretsForFloor(Dungeon.depth) + 2
    ...
    override fun nMobs(): Int {
        return when (Dungeon.depth) {
            1 -> 0 //mobs are not randomly spawned on floor 1.
            else -> 7 + Dungeon.depth % 5 + Random.Int(5)
        }
    }
