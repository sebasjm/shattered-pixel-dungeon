...
class WandOfFireblast : DamageWand() {
    ...
    override fun max(lvl: Int): Int {
        return Math.round((5 + 3 * lvl) * Math.pow(1.5, (chargesPerCast() - 1).toDouble())).toInt()
    }
        ...
        for (cell in affectedCells!!) {
            ...
            if (ch !=
                ...
                when (chargesPerCast()) {
                    ...
                    2 -> Buff.affect<Cripple>(ch, Cripple::class.java, 4f)
                    3 -> Buff.affect<Paralysis>(ch, Paralysis::class.java, 4f)
                }//no effects
            }
        }
    }
    ...
    override fun onHit(staff: MagesStaff, attacker: Char, defender: Char, damage: Int) {
        ...
        Blazing().proc(staff, attacker, defender, damage)
    }
