...
class CryptRoom : SpecialRoom() {
    ...
    override fun paint(level: Level) {
        ...
        if (entrance.x == left) {
            Painter.set(level, Point(right - 1, top + 1), Terrain.STATUE)
            Painter.set(level, Point(right - 1, bottom - 1), Terrain.STATUE)
            cx = right - 2
        } else if (entrance.x == right) {
            Painter.set(level, Point(left + 1, top + 1), Terrain.STATUE)
            Painter.set(level, Point(left + 1, bottom - 1), Terrain.STATUE)
            cx = left + 2
    ...
    private fun prize(level: Level): Item {
        ...
        if (!prize!!.cursed) {
            ...
            if (!prize.hasGoodGlyph()) {
                ...
                prize.cursed = prize.cursedKnown
