...
class PillarsRoom : StandardRoom() {
    ...
    override fun paint(level: Level) {
        ...
        if (minDim == 7 || Random.Int(2) == 0) {
            ...
            val pillarInset = if (minDim >= 11) 2 else 1
            val pillarSize = (minDim - 3) / 2 - pillarInset
            ...
            val pillarInset = if (minDim >= 12) 2 else 1
            val pillarSize = (minDim - 6) / (pillarInset + 1)

            val xSpaces = (width() - 2 * pillarInset - pillarSize - 2).toFloat()
            val ySpaces = (height() - 2 * pillarInset - pillarSize - 2).toFloat()
            val minSpaces = Math.min(xSpaces, ySpaces)

            val percentSkew = Math.round(Random.Float() * minSpaces) / minSpaces
